...
object Huffman {
   ...
   */
    def combine(trees: List[CodeTree]): List[CodeTree] = {
      trees match {
        case Nil => trees
        case h :: Nil => trees
        case h :: h2 :: h3 => Fork(h, h2, chars(h) ::: chars(h2), weight(h)+weight(h2)) :: h3
      }
    }
   ...
   */
    def decode(tree: CodeTree, bits: List[Bit]): List[Char] = {
      def acc(t: CodeTree, b: List[Bit]): List[Char] = {
        t match {
          case Leaf(c, _) => if (b.isEmpty) List(c) else c :: acc(tree, b);
          case Fork(l, r, _, _) => if(b.head == 0) acc(l, b.tail) else acc(r, b.tail)
        }
      }
      acc(tree, bits)
    }
   ...
   */
    def encode(tree: CodeTree)(text: List[Char]): List[Bit] = {
      def acc(t: CodeTree)(w: Char): List[Bit] = {
        t match {
          case Leaf(_, _) => List();
          case Fork(l, r, _, _) => if (chars(l).contains(w)) 0 :: acc(l)(w) else 1 :: acc(r)(w)
        }
      }
      text flatMap acc(tree)
    }
   ...
   */
    def mergeCodeTables(a: CodeTable, b: CodeTable): CodeTable = {
      def prepend(bit: Int)(codeTable: CodeTable): CodeTable = {
