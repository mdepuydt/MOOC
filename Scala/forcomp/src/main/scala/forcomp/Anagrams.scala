...
object Anagrams {
  ...
  def wordOccurrences(w: Word): Occurrences = {
    val grouped = w.groupBy(_.toLower).toSeq.sortBy(_._1)
    grouped.map(x => (x._1, x._2.size)).toList
  }

  /** Converts a sentence into its character occurrence list. */
  def sentenceOccurrences(s: Sentence): Occurrences =
    s.flatMap(wordOccurrences).groupBy(_._1).map(x => (x._1, x._2.map(_._2).sum)).toList sorted
  ...
  lazy val dictionaryByOccurrences: Map[Occurrences, List[Word]] = {
    val res = dictionary map (x => (wordOccurrences(x), x))
    res.groupBy(_._1).map(x => (x._1, x._2.map(_._2)))
  }
   ...
   */
  private def removeZeros(list: Occurrences) = list filter (_._2 != 0)
  ...
  /** Returns a list of all anagram sentences of the given sL22wBh839N1rUt9sentence.
   *
